...
exports.getUserCache = async(req, res) => {
  try{
    const { userId, productId } = req.query;

    const isUserAvail = await userModel.findById(userId);
    const isProdutAvail = await productModel.findById(productId);
    if(!isUserAvail)
      return res.status(404).json({ message: "No user Have our records "});
    if(!isProdutAvail)
      return res.status(404).json({ message: "Invalid Product"});

    const userCache = await userCacheModel.find({ userId: userId, productId: productId});
    const stickerId = userCache.find((usrCh) => usrCh.productId?.toString() === productId?.toString());
    return res.status(200).json({
      sticker: stickerId
    })
